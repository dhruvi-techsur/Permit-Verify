...
# 0.0.0.0 binding for child containers. The per-stack env block is
# intentionally EMPTY for this entry (catalog: compose).
#
...
# sandbox via its external Daytona hostname) will NOT see it. This project's
# docker-compose.yml publishes backend (3000) and frontend (5173) as
# "<port>:<port>" (no host-loopback prefix), so this does not apply here.
...
# (compose: none — docker compose itself handles image build/pull.)
...
# compose (Shape A): compose owns its own multi-service orchestration, signal
# propagation, and per-service log prefixing — from the wrapper's perspective
# `docker compose up --build` is a single foreground process. `--build` is
# load-bearing (not an optimization): this project is actively modified
# between boots, and a plain `docker compose up` would keep serving a stale
# image after a source fix lands. Docker's layer cache keeps the rebuild cheap
