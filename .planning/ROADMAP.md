...
Plans:
- [ ] 01-01-PLAN.md — Docker Compose dev stack + PostgreSQL schema migration + idempotent seed (3 roles)
- [ ] 01-02-PLAN.md — Vite/React/TypeScript scaffold + full UX-05 design system (Tailwind tokens, typed constants)
- [ ] 01-03-PLAN.md — NestJS backend scaffold + all 7 auth endpoints + integration test suite
- [ ] 01-04-PLAN.md — Auth pages (Login, Register, Forgot/Reset Password) + shared UI components + Playwright tests
- [ ] 01-05-PLAN.md — Zustand auth store + Axios interceptors + router + app shell (Sidebar + Header) + full e2e tests
...
Plans:
- [ ] 04-01-PLAN.md — NestJS Dashboard module: 3 role-specific endpoints (applicant/reviewer/admin) with SQL aggregations + integration tests
- [ ] 04-02-PLAN.md — Frontend data layer: TypeScript types + API client + Zustand store + role-aware useDashboard hook
- [ ] 04-03-PLAN.md — Applicant & Reviewer dashboard pages + shared components (DashboardStatCard, StatusDistributionChart, RecentActivityFeed) + router/sidebar wiring + Playwright tests
- [ ] 04-04-PLAN.md — Admin dashboard page + ReviewerWorkloadTable + Playwright tests
...
- [ ] 05-05-PLAN.md — Structural WCAG fixes: SkipNav component, AppShell integration, focus traps in modals, auth page form labels
- [ ] 05-06-PLAN.md — WCAG aria-label sweep across all feature pages + @axe-core/playwright automated accessibility test suite
